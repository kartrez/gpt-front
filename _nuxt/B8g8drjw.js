...
}`,kt={__name:"doc-api",async setup(Tt){let h,w;$e({title:"API документация - GPT-Chat",description:"Документация API GPT-Chat. Информация о доступе к API, эндпоинтах, форматах запросов и ответов для интеграции с нашими сервисами.",keywords:"API, документация, интеграция, эндпоинты, разработка, GPT-Chat API",robots:"index, follow"});const he=Ue(),fe=Ye(),ye=T(()=>fe.value&&he.loggedIn),{mobile:D}=Ke(),S=A("text"),M=A("curl"),V=A("curl"),J=A("curl"),F=A("curl"),y=A(null),W=A(null),R=new Le,{data:K}=([h,w]=U(()=>R.getList()),h=await h,w(),h),{data:Q}=([h,w]=U(()=>R.getImageGenerationList()),h=await h,w(),h),{data:be}=([h,w]=U(()=>R.getVideoGenerationList()),h=await h,w(),h),{data:ve}=([h,w]=U(()=>R.getAudioGenerationList()),h=await h,w(),h),B=n=>{const e=new Set,c=[];for(const g of n){const d=g.split("/")[0];if(!e.has(d)&&(e.add(d),c.push(g),c.length===xt))break}return c},Z=n=>Math.max(0,n.length-B(n).length),O=(n,e="",c="")=>{const g=n.value?.length||0;return g===0?"":e?`${e} ${g}${c}`:` (${g})`},_e=[{code:400,when:"Неверный запрос: неизвестная модель, пустой или слишком длинный промпт, неподдерживаемый файл, нарушение любых ограничений полей"},{code:401,when:"Нет заголовка Authorization, ключ не найден, отозван или просрочен"},{code:402,when:"Не хватает баланса на оценочную стоимость запроса. В теле есть balance, estimatedCost и shortfall"},{code:403,when:"Ключ есть, но доступа к этому ресурсу нет"},{code:404,when:"Объект не найден: чужая или удалённая коллекция, истёкшая задача генерации"},{code:429,when:"Слишком часто. Действует на распознавание речи, презентации и разбор брендбука"},{code:500,when:"Ошибка на нашей стороне — запрос можно повторить"}],xe=T(()=>(K.value||[]).map(n=>n.name)),ke=n=>{const e=new Set(xe.value);return n.filter(c=>e.has(c))},X=T(()=>(K.value||[]).filter(n=>(n.input_modalities||[]).includes("image")).map(n=>n.name)),Te=T(()=>B(X.value)),ee=T(()=>Z(X.value)),te=T(()=>(Q.value||[]).map(n=>n.name)),we=T(()=>B(te.value)),ae=T(()=>Z(te.value)),Ae=[{ratio:"1:1",size:"1024×1024"},{ratio:"2:3",size:"832×1248"},{ratio:"3:2",size:"1248×832"},{ratio:"3:4",size:"864×1184"},{ratio:"4:3",size:"1184×864"},{ratio:"4:5",size:"896×1152"},{ratio:"5:4",size:"1152×896"},{ratio:"9:16",size:"768×1344"},{ratio:"16:9",size:"1344×768"},{ratio:"21:9",size:"1536×672"}],qe=[{size:"0.5K",label:"черновик"},{size:"1K",label:"стандарт"},{size:"2K",label:"высокое"},{size:"4K",label:"максимум"}],Pe=T(()=>ke(["qwen/plus-latest","qwen/max-latest","google/gemini-flash-latest","deepseek/deepseek-pro-latest","anthropic/claude-sonnet-latest","openai/gpt-5.6-terra"])),b={text:{curl:`curl https://api.gpt-chat.by/api/chat/completions \\
